...
| D23 | Catalogue rot | **`build_mo.py` refuses to compile**, in the build and in CI alike, with `--allow-drift` as a local override | Asked for as an error in both places rather than a report. Every packaging script, the PyInstaller spec and both workflows already run that one command, so it is the only single point that reaches both, and it is the last moment before a half-empty catalogue is sealed into an installer. Two conditions stop it: a `_()` in the source that never reached the template, and a catalogue that is not fully translated. The second is strict on purpose — all 33 stood at 426 of 426 when this landed, so completeness is a fact rather than a hope — and it is the half to soften first, to a floor per language, if it proves too restrictive. A report was rejected because a report nobody reads is the failure this exists to prevent. **On trial.** |
| D24 | What keys a catalogue | **The English text, and no fuzzy matching** | The alternative is a symbolic identifier — `unlock.menu` rather than the sentence — which makes editing English prose free. It was rejected for the reason it is usually chosen: with an identifier the 33 translations stay attached to a changed source and go silently stale, which is the same shape of defect as the Hungarian plurals, the `pl` that loaded nothing and the Latin `o` in a Russian word. Those were expensive to find precisely because nothing announced them. Keying on the English makes a changed sentence orphan its translations, which is loud, and loud is what this project has been buying. `sync` also passes `no_fuzzy_matching=True`, so a near-match is not carried across marked fuzzy: a changed string is retranslated rather than inherited. Measured cost of the rule: changing a full stop to an exclamation mark in one guide sentence orphans 33 translations. Accepted deliberately — no drift without having to fix it. Secondary reason: with identifiers the source stops saying what it prints. |
| D25 | Where a compressed page lands | **A new temporary document, with the pages repointed at it** — not the file they already reference, and not the export | Undo is the whole constraint. `UndoManager.snapshot` takes a shallow copy of every `Page`, and a `Page` is a reference into one of `DocumentSet`'s immutable temporary files plus a handful of numbers, so no snapshot holds a single pixel. Rewriting the image XObjects inside the file a page already points at is therefore invisible to undo: every state on the stack, before and after, names that same path, so Undo would faithfully restore page order, rotation and crop while the images stayed degraded — with the menu still offering "Undo Compress". Silent, irreversible, and the version that gets written first, which is why this is settled before the code exists. Doing it at save time instead keeps undo honest only by keeping it ignorant: the thumbnails stop being what gets written, and a lossy operation ends up where nothing can reverse it. Writing a fresh temporary document and repointing `nfile` and `copyname` is what `stamp.py`, `nup.py`, `booklet.py` and `layers.py` already do with generated content, and undo then works because restoring the reference restores the pixels. The cost is disk rather than memory: the original file stays for as long as any undo state names it, so compressing a 200 MB scan means two copies in the per-window temporary directory until the history is cleared. One new document per affected source, with page indices preserved, so `npage` needs no remapping and compressing part of a document stays possible. The feature set and the measurements behind it are in [COMPRESSION.md](completed/COMPRESSION.md). |
