...
**Stage 2 — generate three or four languages at a time.** All 426 messages of a
language in one pass, interface and user guide together, so a bad group is one
`git revert` rather than an excavation. Start with de, fr and es, where the
output is strongest and a mistake is most likely to be spotted and reported.
