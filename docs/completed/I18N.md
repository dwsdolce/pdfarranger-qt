...
**Complete** — the Internationalisation milestone is closed. The policy for machine translation is [D22](../DECISIONS.md); the build refuses to compile a rotted catalogue under [D23](../DECISIONS.md).


The port has been shipping a translated interface that was not one. Four
...
**Seven translated languages could not be chosen.** The Language menu in
Preferences was a hand-written table of thirty, and nothing tied it to the
thirty-three catalogues beside it. Seven had no entry — `ca@valencia`, `eu`,
`is`, `ka`, `oc`, `pl_PL`, `pt_PT` — and three entries had no catalogue. The
worst was Polish: offered as `pl` while the catalogue is `pl_PL`, so choosing
it loaded nothing, left the interface in English, and gave no other way to
reach a translation that had been sitting in the repository all along. The list
is read off the installed catalogues now, and Qt's CLDR data names each one in
its own language, so there is nothing left to keep in step. Three names are
overridden where CLDR answers something the catalogue does not claim, and a
test deletes an override the day Qt agrees with it.

A fifth cause is subtler and had no symptom to notice. `_(label)` on a
...
## Where the catalogues stood

This section and the next are the starting position, kept as measured. They
say 427 messages because that is what the template held at the time; one has
been retired since and it holds 426. Every catalogue now carries all 426 --
see *Stage 2* below.

Measured against the regenerated template: **427 messages, 33 catalogues,
10,387 missing translations** — 10,179 after Pull Request #2 was merged.

| Coverage | Catalogues |
|---|---|
| 80% | ru, once Pull Request #2 was merged — the only one above 40% |
...
Only Russian covers more than 40%, and only since Pull Request #2. The catalogues are
upstream's GTK strings; everything this port added — the menu bar, the bookmark
sidebar, read mode, the whole user guide, all of the PDF24 work — is untranslated
...
- [x] Take Pull Request #2's Russian — the one human-translated catalogue, and the base the
...
Those are ticked here because they never had an Issue — they were found and
fixed before the work was broken into stages, and this is the only record of
them.

**Everything since is tracked on GitHub, and its status is not repeated here:**

- [Issue #3](https://github.com/dwsdolce/pdfarranger-qt/issues/3) — stage 0, the guard tests
- [Issue #4](https://github.com/dwsdolce/pdfarranger-qt/issues/4) — stage 1, the merge tool
- [Issue #5](https://github.com/dwsdolce/pdfarranger-qt/issues/5) — stage 2, filling the catalogues
- [Issue #7](https://github.com/dwsdolce/pdfarranger-qt/issues/7) — stage 3, keeping them filled

So the rule is: **if it has an Issue, GitHub owns whether it is done; if it
never had one, a checkbox here is the record.** This file says what the
approach is and why. Anything written in both places drifts, and this section
did — it carried "Stage 1 … Done." in prose while the Issue said the same
thing, and the two would have parted company the moment one was edited.
...
Both are checks rather than review, and both belong to stage 0
([Issue #3](https://github.com/dwsdolce/pdfarranger-qt/issues/3)).
...
of the messages**, and it is *nearly* the safe part: no mnemonics, no plurals,
one `%` specifier — but 29 of its 37 messages carry HTML markup, which is the
one thing in it that can break and the one thing nobody would see. The
interface is where the rest of the risk is: 75 messages carry a `_` mnemonic, 8
a `%` specifier, 5 are plurals.

That split looked like a reason to translate the interface across every
language first and the guide afterwards. It is not, once the markup is checked
— see *How to attack it* below.
...
German was the proof: synced against the template it went from 238 messages to
426 — two thirds of the interface had not been *untranslated* there so much as
absent, with no entry for a translation to attach to. Ten strings were then
filled, and four deliberately broken ones refused: a dropped accelerator, a
`%d` returned as `%s`, a plural with one form where German wants two, and an
empty string. `Add Page Numbers…` is still English in German for exactly that
reason, which is the system working where it can be seen.

**Stage 2 — generate three or four languages at a time — complete.** All 426
messages of a language in one pass, interface and user guide together, so a bad
group is one `git revert` rather than an excavation. It started with de, fr and
es, where the output is strongest and a mistake is most likely to be spotted
and reported, and ended eight groups later with eu, oc and ka. **All 33
catalogues are at 426 of 426**, 14,058 translated messages. 10,175 of those
are machine output carrying the `machine translation, unreviewed` comment;
the other 3,883 are human translations that were already there and were never
overwritten. Russian keeps the most of its own, 341 of 426.

Three things the guards caught that a reviewer would not have. Russian carried
`% oт высоты` with a Latin `o` where Cyrillic belongs, identical on screen and
invisible in a diff; that produced the script check, which then had to be
narrowed when Arabic proved it wrong — `وPage` and `لـPDF` are correct, because
Arabic proclitics attach to a Latin word exactly as Korean particles do. Only
Latin, Cyrillic and Greek share letterforms, so only those three can hide a
homoglyph. Second, the Language menu in Preferences offered a `pl` no catalogue
answered to. Third, the accelerator ratchet: **106 accelerators were moved**
across the groups, every one of them a collision this work introduced,
and dropped accelerators fell from 106 across 16 languages to 87 across 13
while the number of translated entries carrying one more than doubled.

This was first planned the other way round — all 389 interface messages across
every language, then the 37 guide messages afterwards, on the grounds that the
guide is 56% of the words for 9% of the messages and could slip a release. What
changed it was measuring: **38 messages carry HTML markup and the validator had
no opinion about tags**, so a dropped `</b>` would have been written into 32
catalogues and found only by somebody reading Help in a language nobody here
reads. Adding that check made a single pass per language safe, and a single pass
keeps terminology consistent within a language — which is where machine
translation is weakest. The groups are the only axis now; there is no second
one.
...
## Changing the language used to need a restart — **complete**
...
caught the third fault in Pull Request #1 on its own, along with any future one of the same
...
substance is on main, adapted — it was raised against 2026-09-04 and the PDF24 work
...
Pull Request #2 (Lumenman) rebuilt `po/ru.po` on upstream's August refresh and added this
...
fill in around. The 85 still missing there are the PDF24 work's own commands and the
